    ...
    suspend fun uninstallPlugin(internalName: String, cacheDir: String) = withContext(Dispatchers.IO) {
        ...
        if (dir.exists()) {
            dir.listFiles { f ->
                ...
                base == sanitized || base == internalName.lowercase()
            }?.forEach { runCatching { it.delete() } }
        }
        val inst = RepoState.installedPlugins.value.find { it.internalName.equals(internalName, ignoreCase = true) }
        if (inst != null) {
        ...
        if (jarsDir.exists()) {
            jarsDir.listFiles { f ->
                val base = f.nameWithoutExtension.lowercase().removeSuffix("-jvm4")
                base == sanitized || base == internalName.lowercase()
            }?.forEach { runCatching { it.delete() } }
        }
